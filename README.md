...
A snakemake workflow for viral strain detection with haplotype
reconstruction using RVHaplo.
```
...
| `{sample}_VIRAL_TARGETS_FOUND` **OR** `{sample}_NO_VIRAL_TARGETS` | A flag file indicating if viral targets are found. Used to determine if RVHaplo haplotype reconstruction should run for this sample. |
| `{sample}_viral_targets.log` | All viral targets before applying filtering. |
| `{sample}_viral_refs.done` | A flag file ensuring viral target genomes have been split into individual FASTA files. Only generated for samples with viral targets found. |
| `{sample}_viral_refs/` | Directory containing individual FASTA files for each viral target, used as references for RVHaplo. Only generated for samples with viral targets found. |
| `{sample}_align_to_targets.done` | A flag file ensuring reads have been aligned to each individual viral target. Only generated for samples with viral targets found. |
